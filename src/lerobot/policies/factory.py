...
def resolve_policy_features(
    features: dict[str, Any],
    rename_map: dict[str, str] | None = None,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Build policy input/output features from dataset or env features.

    Always derives both dicts from ``features`` so finetuning from a pretrained
    checkpoint (e.g. ``smolvla_base`` with 6-DoF state) does not keep stale
    ``input_features`` shapes. Optional ``rename_map`` rewrites observation keys
    (e.g. ``observation.images.top`` → ``observation.images.camera1``).
    """
...
def _log_feature_overrides(
    ...
    *,
    ...
    if not previous:
        return
    for key, new_ft in updated.items():
        old_ft = previous.get(key)
        if old_ft is None:
            logging.info("Policy %s: adding %s shape=%s from dataset/env", kind, key, tuple(new_ft.shape))
            continue
        if tuple(old_ft.shape) != tuple(new_ft.shape):
            logging.warning(
                "Policy %s: overriding %s shape %s → %s to match dataset/env "
                "(pretrained feature metadata was stale).",
                kind,
                key,
                tuple(old_ft.shape),
                tuple(new_ft.shape),
...
def make_policy(
    ...
    previous_input_features = dict(cfg.input_features) if cfg.input_features else None
    previous_output_features = dict(cfg.output_features) if cfg.output_features else None
    input_features, output_features = resolve_policy_features(features, rename_map=rename_map)
    _log_feature_overrides(previous_input_features, input_features, kind="input_features")
    _log_feature_overrides(previous_output_features, output_features, kind="output_features")
    # Always sync with dataset/env. Pretrained configs (e.g. smolvla_base state=6) must not
    # keep stale shapes when finetuning on a different embodiment (e.g. UnoArm state=16).
    ...
    # from_pretrained may reload config fields; keep dataset/env feature metadata authoritative.
